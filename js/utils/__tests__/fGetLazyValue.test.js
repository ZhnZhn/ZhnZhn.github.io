...
var _fGetLazyValue = require("../fGetLazyValue");
describe("fGetLazyValue", () => {
  const fn = _fGetLazyValue.fGetLazyValue;
  test("should return function that get value created only once", () => {
    const _crValue = jest.fn(() => ({
        v: "value"
      })),
...
describe("fGetLazyValueAsync", () => {
  const fn = _fGetLazyValue.fGetLazyValueAsync;
  test("should return promisify value", async () => {
    const _value = {
        v: "value"
      },
      _crValue = jest.fn(() => Promise.resolve(_value)),
      _getValue = fn(_crValue);
  ...
  test("should try recreat value in case promisify set void 0", async () => {
    let i = 0;
    const _crValue = jest.fn(() => Promise.resolve(i < 2 ? (i++, void 0) : i)),
      _getValue = fn(_crValue);
    let _result = await _getValue();
    expect(_result).toBe();
    _result = await _getValue();
