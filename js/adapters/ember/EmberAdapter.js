...
const SOURCE_FOSSIL = 'Fossil',
  ...
  _crTotalData = (json, metric) => {
    ...
    return _getObjectKeys(_hm).map(key => [(0, _fnAdapter.ymdToUTC)(parseInt(key, 10)), _hm[key]]);
  },
  _crSourceData = (json, metric, source) => json.reduce((data, item) => {
    if (item.variable === source) {
      data.push([(0, _fnAdapter.ymdToUTC)(item.year), item[metric]]);
    }
    return data;
  }, []);
const crData = (json, options) => {
  const {
    ...
    data = (0, _fnAdapter.isTotalData)(source) ? _crTotalData(json, metric) : _crSourceData(json, metric, source);
