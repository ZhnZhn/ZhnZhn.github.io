...
const crItemCaption = (_ref, json) => {
  ...
  return `${(0, _AdapterFn.crShortItemCaption)(itemCaption)} ${(0, _numberFormatFn.formatAllNumber)(_getByProps(json, "net_assets"), "")}`;
};
  ...
  (0, _isTypeFn.getObjectKeys)(asset_allocation).forEach(key => {
    if (key !== DOMESTIC_EQUITIES) {
      ...
      if ((0, _isTypeFn.isPositiveNumber)(value)) {
        data.push((0, _CategoryFn.crCategoryPoint)(value, key === FOREIGN_EQUITIES ? FOREIGN_EQUITIES_SHORT_FORM : (0, _strFn.toUpperCaseFirst)(key)));
      }
    }
  });
