...
var _flow = _interopRequireDefault(require("../../utils/flow"));
...
const _addExDividend = (exDividendIndex, result) => {
  ...
  if (point[exDividendIndex] !== 0) {
    ...
    if (!(0, _NdlFn.isPrevDateAfter)(dataExDividend, x, 14)) {
  ...
  if (seriaColumnNames[0] === 'All') {
    ...
    for (; i < max; i++) {
      const columnName = seriaColumnNames[i],
        columnIndex = (0, _NdlFn.findColumnIndex)(columnNames, columnName);
      if (columnIndex) {
...
const _crPointFlow = (json, option) => {
  const yPointIndex = (0, _NdlFn.getDataColumnIndex)(json, option),
    fnStep = [_convertToUTC, _checkExtrems, _addToSeria],
    columnNames = (0, _NdlFn.getColumnNames)(json),
    open = (0, _NdlFn.findColumnIndex)(columnNames, _C.OPEN),
    _closeIndex = (0, _NdlFn.findColumnIndex)(columnNames, _C.CLOSE),
    close = !_isUndef(_closeIndex) ? _closeIndex : (0, _NdlFn.findColumnIndex)(columnNames, _C.PRICE),
    low = (0, _NdlFn.findColumnIndex)(columnNames, _C.LOW),
    high = (0, _NdlFn.findColumnIndex)(columnNames, _C.HIGH),
    volume = (0, _NdlFn.findColumnIndex)(columnNames, _C.VOLUME),
    exDividend = (0, _NdlFn.findColumnIndex)(columnNames, _C.EX_DIVIDEND),
    splitRatio = (0, _NdlFn.findColumnIndex)(columnNames, _C.SPLIT_RATIO),
    result = {
      yPointIndex,
  ...
  return [(0, _flow.default)(...fnStep), result];
};
const crAreaData = (json, option) => {
  const [callPointFlow, result] = _crPointFlow(json, option),
    points = (0, _NdlFn.getData)(json).sort(_compareByFn.compareByDate);
  let i = 0,
    _max = points.length;
  for (; i < _max; i++) {
