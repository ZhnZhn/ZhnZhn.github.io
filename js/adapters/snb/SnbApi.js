...
const _isDimensionId = id => id === "D0" || id === "D1",
  _crItem = ({
    id,
    v
  }) => _isDimensionId(id) ? `${id}(${v})` : v;
const _crDimSel = options => options.items.map(_crItem).sort().join(',');
const getRequestUrl = options => `${options.proxy}${API_URL}/${options.dfId}/data/json/en?dimSel=${_crDimSel(options)}&fromDate=${options.fromDate}`,
  checkResponse = (0, _AdapterFn.fCheckResponse)(_fnAdapter.getTimeSeriesValues),
  SnbApi = (0, _ApiFn.crProviderApi)(getRequestUrl, checkResponse);
