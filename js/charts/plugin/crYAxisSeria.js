...
const _getType = chartInst => chartInst?.userOptions?.chart?.type ?? void 0;
const _isCategoryChart = chartInst => {
  const _type = _getType(chartInst);
  return _type === 'bar' || _type === 'column';
};
const _isCategoryData = data => _isArr(data) && data[0]?.c;
const _isCategoryCase = (chartInst, data) => _isCategoryChart(chartInst) && _isCategoryData(data);
const _getCategories = chartInst => chartInst?.userOptions?.xAxis?.categories ?? void 0;
...
var _default = exports.default = crYAxisSeria;
