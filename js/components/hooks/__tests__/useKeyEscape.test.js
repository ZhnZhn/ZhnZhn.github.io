...
describe('useKeyEscape', () => {
  test('', () => {
    const onEscape = jest.fn() //1 Test return type
    ,
          {
      result,
      rerender
    ...
    (0, _reactHooks.act)(() => _onKeyEscape(_eventEscape));
    ...
    expect(onEscape.mock.calls[0][0]).toBe(_eventEscape); //2.2 KeyDown not Escape

    const _eventNotEscape = {
      keyCode: 20
    };
    (0, _reactHooks.act)(() => _onKeyEscape(_eventNotEscape));
