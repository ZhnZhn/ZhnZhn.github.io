...
describe("useSelectItem", () => {
  test("should return tuple with refItems, hSelect, tupleFilter", () => {
    const setChartConfigFromItem = jest.fn(),
      {
        result
      } = (0, _react.renderHook)(() => (0, _useSelectItem.default)(setChartConfigFromItem));
    ...
    expect(_getRefItems(result).current).toEqual([{
      ..._item1,
      id: id1
    }]);
    ...
    (0, _react.act)(() => _hSelect(id1, 0, _item2));
    expect(_getRefItems(result).current).toEqual([{
      ..._item2,
      id: id1
    }]);
    ...
    (0, _react.act)(() => _hSelect(TABLE_ID, 1, _itemTable));
    expect(_getRefItems(result).current).toEqual([void 0, {
      ..._itemTable,
      id: TABLE_ID
    }]);
