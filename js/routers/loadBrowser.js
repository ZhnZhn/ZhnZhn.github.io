...
const loadBrowserComp = exports.loadBrowserComp = (0, _crRouter.crGetRoute)({
  ...
  get [_BrowserType.BT_WATCH_LIST]() {
    ...
    if (process.env.NODE_ENV === "_development") {
      return Promise.resolve().then(() => _interopRequireWildcard(require("js/components/watch-browser/WatchBrowser.js"))).then(_asyncFn.getModuleDefault).catch(_asyncFn.throwErrOffline);
    }
    /*eslint-enable no-undef */
    return Promise.resolve().then(() => _interopRequireWildcard(require(/* webpackChunkName: "watch-browser" */
    /* webpackMode: "lazy" */
    "../components/watch-browser/WatchBrowser"))).then(_asyncFn.getModuleDefault).catch(_asyncFn.throwErrOffline);
  },
  get _BrowserSlider() {
    /*eslint-disable no-undef */
    if (process.env.NODE_ENV === "_development") {
      return Promise.resolve().then(() => _interopRequireWildcard(require("js/components/browser-slider/BrowserSlider.js"))).then(_asyncFn.getModuleDefault).catch(_asyncFn.throwErrOffline);
    }
    /*eslint-enable no-undef */
    return Promise.resolve().then(() => _interopRequireWildcard(require(/* webpackChunkName: "browser-slider" */
    /* webpackMode: "lazy" */
    "../components/browser-slider/BrowserSlider"))).then(_asyncFn.getModuleDefault).catch(_asyncFn.throwErrOffline);
  },
