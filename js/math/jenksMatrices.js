...
function jenksMatrices(data, nClasses) {
  ...
  const varianceCombinations = []; // loop counters

  let i, j; // the variance, as computed at each step in the calculation

  let variance = 0; // Initialize and fill each matrix with zeroes

  for (i = 0; i < data.length + 1; i++) {
    const tmp1 = [];
    const tmp2 = []; // despite these arrays having the same values, we need
  ...
  for (let l = 2; l < data.length + 1; l++) {
    ...
    let sum = 0; // `ZSQ` originally. the sum of squares of values seen
    // thus far

    let sumSquares = 0; // `WT` originally. This is the number of

    let w = 0; // `IV` originally

    let i4 = 0; // in several instances, you could say `Math.pow(x, 2)`
    ...
    for (let m = 1; m < l + 1; m++) {
      ...
      const val = data[lowerClassLimit - 1]; // here we're estimating variance for each potential classing
...
var _default = jenksMatrices;
exports.default = _default;
//# sourceMappingURL=jenksMatrices.js.map