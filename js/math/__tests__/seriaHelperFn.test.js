...
describe('getZeroCountFromStart', () => {
  ...
  test('should return number of points with y 0 or null from data start', () => {
    ...
    expect(fn(dataArr, (0, _seriaHelperFn.fGetY)(dataArr))).toBe(2);
    const dataArr2 = [[1, 0], [2, null], [-1, 1]];
    expect(fn(dataArr2, (0, _seriaHelperFn.fGetY)(dataArr2))).toBe(2);
    const dataObj = [{
      x: 1,
      y: 0
    }, {
    ...
    expect(fn(dataObj, (0, _seriaHelperFn.fGetY)(dataObj))).toBe(2);
  });
});
describe('getZeroIndexFromEnd', () => {
  ...
  test('should return index of last y 0 or null from data end', () => {
    ...
    expect(fn(dataArr, (0, _seriaHelperFn.fGetY)(dataArr))).toBe(1);
    const dataObj = [{
      x: -1,
      y: -1
    }, {
    ...
    expect(fn(dataObj, (0, _seriaHelperFn.fGetY)(dataObj))).toBe(1);
  });
});
