...
const {
  ...
  _crP1,
  _crExpectedPoint
} = _categoryHelpers.default; //Diff S1-S2

describe('categoryDiff', () => {
  const sc = '#111',
        rc = '#222';
  test('should return array with correct diff values', () => {
    const [d1, d2, expectedResult] = _crParams([['P1', 10, 5, 5], ['P2', 10, 0, 10], ['P3', 10, null, 0]], {
      sc,
      rc
    });

    expect((0, _categoryFn.categoryDiff)(d1, d2, {
      sc,
      rc
    })).toEqual(expectedResult);
  });
  test('should return arr with 0 y values for not match points', () => {
    expect((0, _categoryFn.categoryDiff)([_crP1('A1', 10, sc)], [], {
      sc,
      rc
...
describe('categoryRate', () => {
  ...
  test('should return array with rate S1/S2 values', () => {
    const [d1, d2, expectedResult] = _crParams([['P1', 10, 5, 2], ['P2', 10, 0, 0]], {
      sc,
      rc
    });

    expect((0, _categoryFn.categoryRate)(d1, d2, {
      sc,
      rc
    })).toEqual(expectedResult);
  });
  test('should return arr with 0 values for not match points', () => {
    expect((0, _categoryFn.categoryRate)([_crP1('A1', 10, sc)], [], {
      sc,
      rc
    })).toEqual([_crExpectedPoint('A1', 0, rc)]);
  });
...
describe('categoryRoc', () => {
  test('should return arr with correct roc values', () => {
    const sc = '#111',
          rc = '#222',
          [d1, d2, expectedResult] = _crParams([['P1', 10, 5, 100], ['P2', 9, 6, 50], ['P3', 5, 10, -50], ['P4', 0, 9, -100], ['P5', 100.01, 100, 0.01], ['P6', 99.99, 100, -0.01], ['P7', 0, 1, -100], ['P8', 0, -1, 100]], {
      sc,
      rc
    });

    expect((0, _categoryFn.categoryRoc)(d1, d2, {
      sc,
      rc
    })).toEqual(expectedResult);
  });
  test('should return arr with null values for not match points', () => {
    const sc = '#111',
          rc = '#222';
    expect((0, _categoryFn.categoryRoc)([_crP1('A1', 3, sc)], [], {
      rc,
      sc
