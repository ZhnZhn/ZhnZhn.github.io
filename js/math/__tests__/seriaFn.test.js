...
const _crInArrOfObj = arr => arr.map((v, i) => ({
  x: i + 1,
  y: v
}));

const _crInArr = arr => arr.map((v, i) => [i + 1, v]);

const _crOutArr = arr => arr.map((v, i) => [i + 2, v]);
...
describe('changesBetween', () => {
  const fn = _seriaFn.changesBetween;
  ...
  test('should return empty arr in edge case for data', () => {//expect(fn()).toEqual([])
    //_testFn(fn, [], [])
  });
});
describe('normalize', () => {
  const fn = _seriaFn.normalize;
...
describe('filterTrimZero', () => {
  const fn = _seriaFn.filterTrimZero;
