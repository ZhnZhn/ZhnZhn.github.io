"use strict";

exports.__esModule = true;
exports.default = void 0;

/*
 * Pull Breaks Values for Jenks
 *
 * the second part of the jenks recipe:
 * take the calculated matrices
 * and derive an array of n breaks.
 *
 * @private
 */
function jenksBreaks(data, lowerClassLimits, nClasses) {
  const kclass = [];
  let countNum = nClasses;
  let k = data.length; // the calculation of classes will never include the upper
  // bound, so we need to explicitly set it
  //kclass[nClasses] = data[data.length - 1];

  kclass[nClasses] = data[k - 1]; // the lowerClassLimits matrix is used as indices into itself
  // here: the `k` variable is reused in each iteration.

  while (countNum > 0) {
    kclass[countNum - 1] = data[lowerClassLimits[k][countNum] - 1];
    k = lowerClassLimits[k][countNum] - 1;
    countNum--;
  }

  return kclass;
}

var _default = jenksBreaks;
exports.default = _default;
//# sourceMappingURL=jenksBreaks.js.map