...
describe('removeConfig', ()=>{
  const fn = removeConfig;
  test('should remove config from configs by id', ()=>{
    const _chartsConfig = _crChartsConfig(CHART_TYPE)
    const _idOrKey = 'k1'

    expect(isChartExistImpl(_chartsConfig, CHART_TYPE, _idOrKey)).toBe(true)
    const {
      chartSlice,
      isRemoved
    ...
    expect(isChartExistImpl(_chartsConfig, CHART_TYPE, _idOrKey)).toBe(false)
