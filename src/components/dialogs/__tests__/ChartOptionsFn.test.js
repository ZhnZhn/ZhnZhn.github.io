...
describe("crChartOptions", () => {
  ...
  test("should return array with chart options t3ab", () => {
    ...
    expect(fn(
      [{caption: "Item1"},{caption: "Item2"}],
      "t3ab"
    )).toEqual(chartOptionsT3ab)
  })
  ...
  test("should return array with chart options t3c2", () => {
    const chartOptionsT3c2 = [
      ...
      {caption: "Column: By Metric", value: "COLUMN_SET", dim: "Metric"},
      {caption: "Bar: By Metric", value: "BAR_SET", dim: "Metric"}
    ];

    expect(fn(
      [{caption: "Item"},{caption: "Metric"}],
      "t3c2"
    )).toEqual(chartOptionsT3c2)
  })
...
describe("isCategoryItem", () => {
  ...
  test("should return boolean isCategoryItem", () => {
    ...
    expect(fn({value: "TREE_MAP_CLUSTER"})).toBe(true)
    expect(fn({value: "BAR_TREE_MAP"})).toBe(true)

    expect(fn({value: "SPLINE"})).toBe(false)
    expect(fn({value: "LINE"})).toBe(false)
    expect(fn({value: "AREA"})).toBe(false)
    expect(fn({value: "COLUMN"})).toBe(false)
  })
  test("should return false in edge cases", () => {
    ...
    expect(fn(()=>{})).toBe(false)
    expect(fn([])).toBe(false)
    expect(fn({a: "TREE_MAP"})).toBe(false)
  })
})
