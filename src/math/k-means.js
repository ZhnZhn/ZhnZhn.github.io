...
//get array index with min value
const mindex = arr => arr
  .reduce((r, v, index) => r[0] < v
     ? r
     : [v, index],
     [Infinity, -1]
   )[1];
...
const Point = function(location) {
  ...
  this.updateLabel = centroids => {
    const distancesSquared = centroids
      .map(centroid => sumOfSquareDiffs(this.location(), centroid.location()));
...
const clusterMaker = {
  ...
  clusters() {
    const pointsAndCentroids = kmeans(
      this.data(), {
      k: this.k(),
  ...
  crUnarySortedCluster(points=[], n=N, iteration=ITERATION){
    this.k(n)
