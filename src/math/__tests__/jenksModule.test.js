import jenksModule from '../jenksModule';
...
const _getItemPrevColor = (
  r,
  index
) => r[index]._prevColor;

describe("addJenksColorTo", () => {
  const fn = jenksModule.addJenksColorTo;
  test("should add jenks color and _prevColor to data items", ()=>{
    const data = [
      {y:1, color: "aa"},{y:1.1, color: "aa"},
      {y:2},{y:2.2},
      {y:3},{y:3.3},
      {y:4},{y:4.4},
      {y:5, color: "ee"},{y:5.1, color: "ee"},
      {y:6},{y:6.1}
    ]
...
describe("removeJenksColorFrom", ()=>{
  const fn = jenksModule.removeJenksColorFrom;
  test("should set _prevColor to data items", ()=>{
    const data = [
      {y: 1, color: "a", _prevColor: "aa"},
      {y: 2, color: "b", _prevColor: "bb"},
      {y: 3, color: "c"},
      {y: 4}
    ]
    , r = fn(data)
    ...
    expect(r[0]).toEqual({
      y: 1, color: "aa", _prevColor: "aa"
    })
    expect(r[1]).toEqual({
      y: 2, color: "bb", _prevColor: "bb"
    })
    expect(r[2]).toEqual({
      y: 3, color: void 0
    })
