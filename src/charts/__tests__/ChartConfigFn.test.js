...
describe('isLineType', ()=>{
  ...
  test('should return boolean isLineType from chart config seria by index 0', () => {
    ...
    expect(fn({series: []})).toBe(false)

    expect(fn({series: [{type: 'spline'}]})).toBe(true)
    expect(fn({series: [{type: 'line'}]})).toBe(true)
    expect(fn({series: [{type: 'area'}]})).toBe(true)

    expect(fn({series: [{type: 'all-other-types'}]})).toBe(false)
    expect(fn({series: [{type: 'bar'}]})).toBe(false)
    expect(fn({series: [{type: 'column'}]})).toBe(false)
  })
})

const _testSeriaConfigTooltipShape = (
  seriaConfig
) => {
...
describe('crSeriaConfig', () => {
  ...
  test('should return initial seria by empty options', () => {
     ...
     expect(_seriaConfig)
       .toMatchObject(_expectedSeriaConfigInitialShape)
...
const _testSeriaConfigPointEventsMouseOver = (
  seriaConfig
) => {
